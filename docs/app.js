...
function newForm(){
  return { id:null, type:'', type_custom:'', world_day:'', title:'', objective:'',
    ...
    photos:[], existing_photo_ids:[], photo_folder_id:'', notes:'',
...
function renderForm(editing){
  ...
  '<div class="card">'+
    ...
    '<div class="grid">'+
      ...
      execField+
      '<div class="field"><label>عنوان الفعالية'+reqMark('title')+'</label>'+
...
function collectForm(){
  ...
  FORM.world_day=val('f_world'); FORM.title=val('f_title');
...
function missingFormFields(){
  ...
  if(FORM.type==='يوم عالمي' && isRequiredField('world_day') && !has(FORM.world_day)) add('world_day','f_world');
  if(!has(FORM.title)) add('title','f_title');
...
function onSaved(msg){ invalidateCaches(); updatePending(); toast(msg,'ok'); show('records'); }
...
function renderRecords(){
  var years=[]; var y=new Date().getFullYear();
  for(var i=0;i<4;i++) years.push(y-i);
  ...
  '<div class="card">'+
    ...
    '<div class="filters">'+
      '<div class="field"><label>السنة</label><select id="fl_year" onchange="loadRecords(true)">'+
        '<option value="">الكل</option>'+years.map(function(yy){return '<option value="'+yy+'">'+yy+'</option>';}).join('')+'</select></div>'+
      '<div class="field"><label>الشهر</label><select id="fl_month" onchange="loadRecords(true)">'+
        '<option value="">الكل</option>'+months.map(function(m,i){return i?'<option value="'+i+'">'+m+'</option>':'';}).join('')+'</select></div>'+
...
function loadRecords(force){
  var year=val('fl_year'), month=val('fl_month');
  var key=(year||'')+'|'+(month||'');
...
function viewActivity(id){
  ...
  var photos=(o.photo_ids||[]).map(function(pid){
    return '<div class="photo-thumb"><a href="https://drive.google.com/file/d/'+pid+'/view" target="_blank">'+
      '<img loading="lazy" src="https://drive.google.com/thumbnail?id='+pid+'&sz=w200"></a></div>';
  }).join('');
  var body=
    dl('النوع', o.type==='يوم عالمي'&&o.world_day?((o.display_type||o.type)+' — '+o.world_day):(o.display_type||o.type))+
    dl('العنوان', o.title)+ dl('الهدف', o.objective)+ dl('الفئة المستهدفة', o.target_groups)+
...
function paintDash(d){
  ...
  '<div class="card">'+
    ...
    '<p class="sub">'+(USER.role==='admin'?'ملخّص جميع الفعاليات.':'ملخّص فعالياتك.')+'</p>'+
    '<div class="kpis">'+
...
function showDashItems(dimension,key){
  ...
  run('getDashboardItems',USER.no,dimension,key).then(function(list){
    if(!list.length){
...
function renderAdmin(){
  ...
  '<div class="card"><h2>التصدير</h2>'+
    '<p class="sub">تصدير PDF متاح من زر PDF داخل سجل الفعاليات. تصدير Excel سيُفعّل عند إضافة قالب Excel الرسمي.</p>'+
    '<button class="btn btn-ghost" disabled>Excel بانتظار القالب</button>'+
  '</div>';
}
