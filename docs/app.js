...
function invalidateCaches(){ REC_CACHE = {}; DASH_CACHE = {}; }
...
function renderTabs(){
  ...
  document.getElementById('tabsBar').innerHTML=tabs.map(function(t){
    return '<button class="tab" data-tab="'+t.id+'" onclick="show(\''+t.id+'\')">'+t.label+'</button>';
  }).join('');
}
...
function periodControlsHtml(prefix){
  ...
  return '<div class="filters period-filters">'+
    '<div class="field"><label>الفترة</label><select id="'+prefix+'_mode" onchange="onPeriodModeChange(\''+prefix+'\',true)">'+
...
function initPeriodControls(prefix){
  var p=currentPeriod();
...
function onPeriodModeChange(prefix, reload){
  var mode=val(prefix+'_mode')||'current_month';
...
function periodFilter(prefix){
  ...
  return {
    ...
    month: mode==='current_month' ? p.month : (parseInt(val(prefix+'_month'),10)||p.month),
...
function newForm(){
  return { id:null, type:'', type_custom:'', world_day:'', title:'', objective:'',
    target_groups:[], event_date:'', location:'', mechanism:'', mechanisms:[],
...
function submitForm(){
  ...
  if(Outbox.available){
    ...
    // 1) حفظ محلي فوري  2) محاولة الرفع
...
function onSaved(msg){ SUBMITTING=false; if(FORM) FORM._new_unsaved=false; invalidateCaches(); updatePending(); toast(msg,'ok'); show('records'); }
function onSavedLocal(){ invalidateCaches(); updatePending();
  SUBMITTING=false;
  toast('حُفظت محليًا وستُرفع تلقائيًا عند توفّر الاتصال.','ok'); show('records'); }
...
function renderRecords(){
  ...
  '<div class="card">'+
    '<h2>سجل الفعاليات</h2>'+
    '<p class="sub">'+(USER.role==='admin'?'جميع الفعاليات المسجّلة.':'فعالياتك المسجّلة.')+'</p>'+
    ...
    '<div class="filters action-filters">'+
      ...
      '<div class="field" style="justify-content:flex-end"><button class="btn btn-excel btn-sm" id="excelBtn" onclick="exportExcel()">تصدير Excel</button></div>'+
    '</div>'+
    '<div id="recList"><div class="loading"><span class="spin"></span> جارٍ التحميل…</div></div>'+
  '</div>';
  initPeriodControls('fl');
  loadRecords();
}
function loadRecords(force){
  var filter=periodFilter('fl');
  var key=periodKey('fl');
  if(!force && REC_CACHE[key]){ paintRecords(REC_CACHE[key]); return; }
  document.getElementById('recList').innerHTML='<div class="loading"><span class="spin"></span> جارٍ التحميل…</div>';
  run('getActivities', USER.no, filter).then(function(list){
  ...
  box.innerHTML=list.map(function(o){
    ...
    return '<div class="rec">'+
      ...
      '<div class="ops">'+
        ...
        '<button class="btn btn-danger-outline btn-sm" onclick="askDelete(\''+o.id+'\')"><span class="ico">×</span> حذف</button>'+
      '</div></div>';
  }).join('');
}
...
function viewActivity(id){
  ...
  var photos=(o.photo_ids||[]).map(function(pid){
...
function openPhotoViewer(ids,index,title){
  PHOTO_VIEW={ ids:(ids||[]).slice(), index:index||0, title:title||'الصور' };
  paintPhotoViewer();
}
...
function paintPhotoViewer(){
  ...
  var body='<div class="photo-viewer">'+
    '<img src="https://drive.google.com/thumbnail?id='+escAttr(pid)+'&sz=w1600" alt="صورة الفعالية">'+
  ...
  openModal(PHOTO_VIEW.title||'الصور', body,
    '<button class="btn btn-ghost" onclick="closeModal()">إغلاق</button>'+
    nav+
    '<a class="btn btn-primary" target="_blank" rel="noopener" href="https://drive.google.com/file/d/'+escAttr(pid)+'/view">فتح الأصل</a>');
}
...
function exportPdf(id){
  ...
  run('exportActivityPdf',id,USER.no).then(function(r){
    if(r&&r.ok){
      toast('تم إنشاء ملف PDF.','ok');
      openModal('تم إنشاء PDF',
        '<p style="margin:0 0 10px">تم إنشاء تقرير «'+esc(o.title)+'». اختر فتحه أو تحميله.</p>'+
        '<div class="hint">'+esc(r.name||'تقرير PDF')+'</div>',
        '<button class="btn btn-ghost" onclick="closeModal()">إغلاق</button>'+
        '<a class="btn btn-print" target="_blank" rel="noopener" href="'+escAttr(r.url)+'">فتح PDF</a>'+
        '<a class="btn btn-primary" target="_blank" rel="noopener" href="'+escAttr(r.downloadUrl||r.url)+'">تحميل PDF</a>');
    }
...
function exportExcel(){
  ...
  run('exportActivitiesExcel', periodFilter('fl'), USER.no).then(function(r){
    if(btn){ btn.disabled=false; btn.textContent='تصدير Excel'; }
    if(r&&r.ok){
      toast('تم إنشاء ملف Excel.','ok');
      triggerDownload(r.downloadUrl||r.url, r.name||'athar.xlsx');
      openModal('تم إنشاء Excel',
        '<p style="margin:0 0 10px">تم إنشاء تقرير Excel للفترة: '+esc(r.period||'')+'</p>'+
        '<div class="hint">عدد السجلات: '+esc(r.count||0)+' · '+esc(r.name||'تقرير Excel')+'</div>',
        '<button class="btn btn-ghost" onclick="closeModal()">إغلاق</button>'+
        '<a class="btn btn-ghost" target="_blank" rel="noopener" href="'+escAttr(r.url)+'">فتح الملف</a>'+
        '<a class="btn btn-excel" target="_blank" rel="noopener" href="'+escAttr(r.downloadUrl||r.url)+'">تحميل Excel</a>');
    } else {
      toast((r&&r.msg)||'تعذّر إنشاء Excel.','err');
    }
  }).catch(function(e){
    if(btn){ btn.disabled=false; btn.textContent='تصدير Excel'; }
...
function loadDashboard(force){
  var key=periodKey('dash');
  var body=document.getElementById('dashBody');
  if(!force && DASH_CACHE[key]){ paintDash(DASH_CACHE[key]); return; }
  if(body) body.innerHTML='<div class="loading"><span class="spin"></span> جارٍ حساب المؤشرات…</div>';
  run('getDashboard', USER.no, periodFilter('dash')).then(function(d){
    DASH_CACHE[key]=d; paintDash(d);
  }).catch(function(e){
...
function renderAdmin(){
  ...
  '<div class="card"><h2>التصدير</h2>'+
    '<p class="sub">تصدير PDF متاح من زر PDF داخل سجل الفعاليات، وتصدير Excel متاح من السجل حسب الفترة المختارة.</p>'+
