...
function loadRecords(force){
  ...
  run('getActivities', USER.no, filter).then(function(list){
    mergePendingRecords(list, filter).then(function(merged){
      REC_CACHE[key]=merged; paintRecords(merged);
    });
  }).catch(function(e){
  ...
  box.innerHTML=list.map(function(o){
    ...
    return '<div class="rec">'+
      ...
      '<div class="ops">'+
        ...
        (!isPending&&USER.role==='admin'?'<button class="btn btn-print btn-sm" onclick="exportPdf(\''+o.id+'\')"><span class="ico">⎙</span> طباعة PDF</button>':'')+
